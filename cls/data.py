...
def translate_pointcloud2(pointcloud):
    xyz1 = np.random.uniform(low=2./3., high=3./2., size=[3])
    translated_pointcloud = np.add(np.multiply(pointcloud, xyz1), xyz2).astype('float32')
    return translated_pointcloud
...
class ScanObjectNN(Dataset):
    def __init__(self, num_points, partition='train', ver='easy', aug="norm"):
        self.num_points = num_points
        self.partition = partition 
        self.aug = aug
        ...
        if ver == 'easy':
            ...
            if self.partition == 'train':
                file = os.path.join(DATA_DIR, "ScanObjectNN/main_split/training_objectdataset.h5")
            else:
                file = os.path.join(DATA_DIR, "ScanObjectNN/main_split/test_objectdataset.h5")
